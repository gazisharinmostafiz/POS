...
<body class="bg-slate-950 text-slate-100 antialiased">
    <main class="mx-auto max-w-6xl px-6 py-8">
        <div class="mb-6 flex items-start justify-between gap-4">
            <div>
                <p class="text-sm font-semibold uppercase tracking-widest text-cyan-300">Tenant backups</p>
                <h1 class="mt-2 text-3xl font-bold">Backups</h1>
                <p class="mt-2 text-slate-400">Create private backups for database records, uploads, invoices, or the full tenant.</p>
            </div>
        ...
        <form method="POST" action="{{ route('tenant.backups.store') }}" class="mb-6 rounded border border-slate-800 bg-slate-900 p-5">
            @csrf
            <div class="grid gap-4 md:grid-cols-3">
                <div>
                    ...
                    <select id="type" name="type" class="mt-2 w-full rounded border border-slate-700 bg-slate-950 px-3 py-2">
                        ...
                        <option value="invoices">Invoice files</option>
                        <option value="full_tenant">Full tenant</option>
                    </select>
                </div>
                            ...
                            <tr>
                                ...
                                <td class="space-x-2 text-right">
                                    @if ($backup->status === 'completed')
                                        <a class="rounded border border-slate-700 px-3 py-1" href="{{ route('tenant.backups.download', $backup) }}">Download</a>
                                        <form method="POST" action="{{ route('tenant.backups.restore', $backup) }}" class="inline">
                                            @csrf
